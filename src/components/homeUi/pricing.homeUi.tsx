  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[-8rem] right-0 w-96 h-96 bg-blue-600/30 rounded-full blur-3xl"></div>
        <div className="absolute bottom-[-8rem] left-0 w-96 h-96 bg-cyan-600/20 rounded-full blur-3xl"></div>
      </div>

      <div className="container mx-auto px-4 md:px-6 relative z-10">
        {/* Header */}
        <div className="max-w-2xl mx-auto text-center mb-20">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-700/40 border border-blue-400/40 mb-6">
          ...
          {plans.map((plan, i) => (
            <div
              ...
              className={`relative transition-all duration-500 ${
                plan.popular
                  ? 'md:scale-105 md:-mt-6'
                  : 'hover:translate-y-[-4px]'
              }`}
            >
              <div
                className={`relative rounded-3xl overflow-hidden shadow-lg backdrop-blur-xl transition-all duration-300 ${
                  plan.popular
                    ? 'border-2 border-blue-500/70 bg-gradient-to-br from-slate-800/90 to-slate-950/70 shadow-blue-600/30'
                    : 'border border-blue-400/30 bg-gradient-to-br from-slate-900/80 to-slate-950/70'
                }`}
              >
                {/* Highlight Badge */}
                <div className="absolute top-4 right-4 text-xs font-semibold text-blue-100 bg-blue-700/40 px-3 py-1 rounded-full border border-blue-400/40">
                  {plan.highlight}
                </div>
